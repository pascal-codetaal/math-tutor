 ...
 */
const getContainerClassName = (totalColumns: number, baseClasses: string = "") => {
...
const PositionedIcon = ({ children }) => (
    <div className="relative h-0">
        <div className="absolute bottom-full first-letter:transform -translate-x-full p-2 bg-white  -mb-7">
            {children}
        </div>
    </div>
);
...
const UnitIndicators = ({ numberOfItems = 5, totalColumns }: { numberOfItems: number, totalColumns: number }) => {

    const topItems = ["E", "T", "H", "D", "TD", "HD", "M"].slice(0, numberOfItems + 1);

    return Array.from({ length: totalColumns }, (_, index) => (
        <div
            key={index}
            className={`p-4  flex justify-center items-center bg-${colors[index]} rounded-lg text-white	font-bold`}
        >
            {topItems[index]}
        </div>
    ));
...
const HelpFieldItem = () => {
    ...
    return (
        <div className="border-2 p-2 flex justify-center items-center rounded-full" onClick={() => inputRef.current?.focus()}>
            <input
                ref={inputRef}
                type="text"
                style={{ direction: "ltr" }}
                ...
                pattern="[0-9]*"
                maxLength={1}
                className="w-8 h-8 border-none text-sm text-center p-0 rounded-full" // Adjusted classes
            />
        </div>
    );
}
...
export const MultiplyComponent = ({ firstMultiplier, secondMultiplier }: { firstMultiplier: number, secondMultiplier: number }) => {
    ...
    const containerClassName = cn(`grid grid-cols-${totalColumns} gap-2`, getContainerClassName(totalColumns));


    return (
        <div className="flex items-center space-x-4 p-16 "> {/* Flex container with vertical center alignment and some spacing */}
            <div className="flex-row">
                ...
                <div style={{ direction: "rtl" }}
                    ...
                    <UnitIndicators numberOfItems={totalColumns} totalColumns={totalColumns} />
                </div>

                <div style={{ direction: "rtl" }}
                    className={cn(containerClassName, "mt-2")}>
                    <MultiplicandStepDisplayRow key={"first"} solution={firstMultiplier} totalColumns={totalColumns} dynamic={true} />
                </div>
                <PositionedIcon><XCircleIcon style={{ height: "2rem", width: "2rem" }} className="h-8 w-8 flex-none text-gray-400" /> </PositionedIcon>
                <div style={{ direction: "rtl" }}
                    className={cn(containerClassName, "mt-2")}>
                    <MultiplicandStepDisplayRow key={"second"} solution={secondMultiplier} totalColumns={totalColumns} dynamic={true} />
                </div>
                {stepSolutions?.length > 1 && (<>
                    <div style={{ direction: "rtl" }}
                        className={cn(containerClassName, "border-t-4 border-black mt-2")}>
                    </div>
                    {stepSolutions.map((solution, index) => <Fragment key={`${index}-${solution}`}><div style={{ direction: "rtl" }}
                        className={cn(containerClassName, "mt-2")}>
                        <MultiplicandStepDisplayRow key={`${index}-${solution}`} totalColumns={totalColumns} solution={solution} dynamic={false} />
                    </div>{index < stepSolutions.length - 1 && <PositionedIcon><PlusCircleIcon style={{ height: "2rem", width: "2rem" }} className="h-8 w-8 flex-none text-gray-400" /></PositionedIcon>}</Fragment>)}</>)}
                <div style={{ direction: "rtl" }}
                    className={cn(containerClassName, "border-t-4 border-double border-black mt-2")}>
                </div>
                <div style={{ direction: "rtl" }}
                    className={cn(containerClassName, "mt-2")}>
                    <MultiplicandStepDisplayRow totalColumns={totalColumns} solution={solution} dynamic={false} />
                </div>
            </div>
            <div className="border-2 p-4 w-32 h-32"> {/* New div with a border and padding */}
...
const MultiplicandStepDisplayRow = ({ totalColumns, solution, dynamic }: { totalColumns: number, solution: string | number, dynamic: boolean }) => {
    const solutionArray = solution.toString().split("").reverse();

    const { showSolution } = useContext(MathContext);
    console.log({ showSolution })

    const RenderItem = ({ value, borderColor }: { value: string, borderColor?: string }) => {
        const inputRef = useRef<HTMLInputElement>(null);
        const color = `border-${borderColor}`
        return (
            <div
                className={`border-2 p-2 flex justify-center items-center rounded-lg ${color}`}
                onClick={() => inputRef.current?.focus()}
            >
                {dynamic ? <span className="text-lg font-bold">{value}</span> : <input
                    type="text"
                    ref={inputRef}
                    ...
                    pattern="[0-9]*"
                    maxLength={1}
                    className={`w-full border-none text-lg text-center p-0 font-bold ${color}`}
                />}
            </div>
        );
    }
    return (<>
        {Array.from({ length: totalColumns }, (_, index) => (
            <RenderItem key={index} value={solutionArray[index]} borderColor={colors[index]} />
        ))}
    </>)
}